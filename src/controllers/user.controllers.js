...
const create = catchError(async(req, res) => {
    ...
    const protected = await bcrypt.hash(password, 10)
    const result = await User.create(
        { email, password: protected, firstName, lastName, country, image, isVerified }
    );
    ...
    await sendEmail({
        ...
        <p>thanks for sign up inuser app</p></br>
        <a href="${link}">${link}</a>
        `
    })

    return res.status(201).json({result, message: "send email"});
});
...
const verifyCode = catchError(async(req, res) => {
    ...
    if(!emailCode) return res.status(401)
...
const login = catchError(async(req, res) => {
    ...
    if(!valid && !user.isVerified) return res.json(401)
